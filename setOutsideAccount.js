...
async function setOutsideAccount(addr,_from,_from_key) {
    ...
    const tx = {
      ...
      'data': photoCoreContract.methods.setOutsideAccount(addr).encodeABI()
    };
...
async function main(addr,_from,_from_key){
    await setOutsideAccount(addr,_from,_from_key);
    console.log('done');
  }
  
main(process.env.a8_address,process.env.a3_address,process.env.a3_key);